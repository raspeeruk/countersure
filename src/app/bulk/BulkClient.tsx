...
export function BulkClient() {
  ...
  async function submit() {
    ...
    setCsvOut(null);
    try {
      const res = await fetch('/api/vat/bulk', {
        method: 'POST',
  ...
  function download() {
    ...
    a.href = url;
    a.download = `countersure-bulk-${Date.now()}.csv`;
      ...
      {summary ? (
        <div className="mt-8">
          ...
          <div className="grid grid-cols-4 gap-px bg-slate-ink/10 border border-slate-ink/10 mb-6">
            ...
            <div className="bg-clearance-white p-4 text-center">
              <p className="font-mono text-xs text-customs-green uppercase">Verified</p>
              <p className="font-heading text-3xl font-black text-customs-green">{summary.verified}</p>
            </div>
            <div className="bg-clearance-white p-4 text-center">
              <p className="font-mono text-xs text-flagged-amber uppercase">Not found</p>
              <p className="font-heading text-3xl font-black text-flagged-amber">{summary.not_found}</p>
            </div>
            <div className="bg-clearance-white p-4 text-center">
